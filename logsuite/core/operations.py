...
def align_properties(
    ...
    if len(prop1.depth) == len(prop2.depth) and np.allclose(
        prop1.depth, prop2.depth, rtol=1e-9, atol=1e-9
    ):
...
class PropertyOperationsMixin:
    ...
    def __neg__(self) -> "Property":
        """Negate: -self"""
        return self._create_result_property(
            name=f"neg_{self.name}", depth=self.depth.copy(), values=-self.values, unit=self.unit
        )

    def __pos__(self) -> "Property":
        """Positive: +self"""
        return self._create_result_property(
            name=f"pos_{self.name}", depth=self.depth.copy(), values=+self.values, unit=self.unit
        )
