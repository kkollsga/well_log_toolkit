...
def example_dataframe_export():
    ...
    df_all = well.data()
    print("All properties (aligned to first property's grid using 'match' method):")
    print(df_all.head())
    print(f"Columns: {list(df_all.columns)}\n")

    # 2. Export with specific reference property (aligned to PHIE's grid)
    df_ref = well.data(reference_property='PHIE')
    print("Using PHIE as reference:")
    print(f"Shape: {df_ref.shape}\n")

    # 3. Include only specific properties
    df_subset = well.data(include=['PHIE', 'SW', 'PERM'])
    print("Only PHIE, SW, and PERM:")
    print(df_subset.head())
    print()

    # 4. Exclude specific properties (useful when you want most properties)
    df_exclude = well.data(exclude=['QC_Flag', 'Temp_Data'])
    ...
    # Note: merge_method controls alignment behavior:
    # - 'match' (default): errors if depths don't align exactly (safest)
    # - 'resample': interpolates to reference depth grid
    # - 'concat': merges all unique depths
    # df_resampled = well.data(merge_method='resample')
    print()

    # 7. Typical workflow: just call data() and it handles everything
